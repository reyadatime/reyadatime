...
export default function CountryHomePage({ params }: PageProps) {
  ...
  // Ensure country code is always lowercase in the URL
  useEffect(() => {
    if (countryCode && countryCode !== countryCode.toLowerCase()) {
      // Redirect to lowercase version of the URL
      const newPath = pathname.replace(`/${countryCode}`, `/${countryCode.toLowerCase()}`);
      router.replace(newPath);
      return;
    }
    
    // Update country context if needed
    if (countryCode && (!currentCountry || currentCountry.code.toLowerCase() !== countryCode.toLowerCase())) {
      changeCountry(countryCode.toLowerCase());
    }
  }, [countryCode, currentCountry, pathname, router, changeCountry]);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(`/${countryCode}/facilities?${queryParams.toString()}`);
  };
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="relative z-10 py-16 px-6 md:py-44 md:px-12 text-white w-full backdrop-blur-sm">
          <div className={`max-w-3xl ${language === 'ar' ? 'mr-auto' : 'ml-0'} ${language === 'ar' ? 'md:mr-24' : 'md:ml-24'}`}>
            ...
            <div className="flex flex-wrap gap-6">
              <Link href={`/${countryCode}/facilities`}>
                <Button 
                  size="lg" 
      ...
      <section className="mb-16">
        <div className="flex justify-between items-center mb-8">
          ...
          <Link href={`/${countryCode}/facilities`} className="text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 text-sm font-medium">
        ...
        <div className="grid grid-cols-3 md:grid-cols-7 gap-6">
          {sportTypes.map((sport) => (
            <Link 
              href={`/${countryCode}/facilities?type=${sport.id}`} 
              key={sport.id}
